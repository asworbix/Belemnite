...
export function verifyClaimedBot(ua, ip) {
    if (!ua)
    ...
    if (!claim)
        return { verified: false };
    if (!ip)
        return { verified: false, reason: 'no-ip' };
    for (const cidr of claim.cidrs) {
        if (ipInCidr(ip, cidr))
            return { verified: true, name: claim.name };
    }
    return { verified: false, reason: 'ip-not-in-range' };
}
function findClaim(ua) {
...
export function listVerifiedBots() {
    return [...VERIFIED];
}
